...
class user:   
    def __init__(self,master):  # return a object with  given name ,an object of class is constructed
    	# Window 
        self.master = master
        # Some Usefull variables # use string variables for user and password seting 
    ...
    def login(self):
        ...
        result = c.fetchall()
        if result:
            self.logf.pack_forget() #linked list 
    ...
    def new_user(self):
    	#Establish Connection to store data
        with sqlite3.connect('Users.db') as db:
            c = db.cursor()
...
class travel:
    ...
    # logouting are exiting cab agent 
        ...
        def Reset():
            ...
            self.txtLuxury.configure(state=DISABLED)
            self.txtSuperluxury.configure(state=DISABLED)
        ...
        def Receiptt():
            if reset_counter == 0 and Firstname.get()!="" and Surname.get()!="" and Address.get()!="" and Postcode.get()!="" and Mobile.get()!="" and Telephone.get()!="" and Email.get()!="":
                ...
                self.txtReceipt1.insert(END,'Normal:\n')
                self.txtReceipt2.insert(END, Normal.get() + "\n")
                self.txtReceipt1.insert(END,'Prime Sedan:\n')
                self.txtReceipt2.insert(END, Luxury.get() + "\n")
                self.txtReceipt1.insert(END,'Premium Sedan:\n')
                self.txtReceipt2.insert(END, Superluxury.get() + "\n")
        ...
        def Kilo():
            if var2.get() == 0:
                ...
                if varl1.get() == "Ameerpet":
                    switch ={"Ameerpet": 10,"Gachibowli": 8,"Madhapur":6,"Kukatpalli": 0}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Gachibowli":
                    switch ={"Ameerpet": 0,"Gachibowli": 2,"Madhapur":5,"Kukatpalli": 10}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Madhupur":
                    switch ={"Ameerpet": 2,"Gachibowli": 0,"Madhapur":3,"Kukatpalli": 8}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Kukatpalli":
                    switch ={"Ameerpet": 5,"Gachibowli": 3,"Madhapur":0,"Kukatpalli": 6}
                    Km.set(switch[varl2.get()])        
        ...
        def selectCar():
            ...
            if carType.get() == 1:
                ...
                self.txtSuperluxury.configure(state = DISABLED)
                Luxury.set("0")
                self.txtLuxury.configure(state = NORMAL)
                Item5 = float(15)
                Luxury.set("Rs "+ str(Item5))
            else:
                self.txtNormal.configure(state =DISABLED)
                Normal.set("0")
                self.txtLuxury.configure(state = DISABLED)
                Luxury.set("0")
                self.txtSuperluxury.configure(state = NORMAL)
                Item5 = float(22)
                Superluxury.set("Rs "+ str(Item5))
   ...
   #================================================mainframe========================================================================
    ...
    #===============================================Cab Information==============================================================
        ...
        self.cboPickup['value']=('','Ameerpet','Gachibowli','Madhapur','Kukatpally')
        self.cboPickup.current(0)
        ...
        self.cboDrop['value']=('','Ameerpet','Gachibowli','Madhapur','Kukatpalli')
        self.cboDrop.current(0)
    ...
    #==========================================================Cabselect=======================================================================
        ...
        self.chkNormal=Radiobutton(Book_Frame,text="Normal Cab",value=1,variable = carType,font=('arial',14,'bold'),command=selectCar).grid(row=0, column=0, sticky=W)
        ...
        self.chkLuxury=Radiobutton(Book_Frame,text="Luxury Cab",value=2,variable = carType,font=('arial',14,'bold'),command=selectCar).grid(row=1, column=0, sticky=W)
        self.txtLuxury= Label(Book_Frame,font=('arial',14,'bold'),width =7,textvariable=Luxury,bd=5, state= DISABLED, justify=RIGHT,bg="white",relief=SUNKEN)
        self.txtLuxury.grid(row=1,column=1)
             
       
        self.chkSuperluxury = Radiobutton(Book_Frame,text="Super luxury Cab",value=3,variable = carType,font=('arial',14,'bold'),command=selectCar).grid(row=2, column=0)
        self.txtSuperluxury = Label(Book_Frame,font=('arial',14,'bold'),width =7,textvariable=Superluxury,bd=5, state= DISABLED, justify=RIGHT,bg="white",relief=SUNKEN)
        self.txtSuperluxury.grid(row=2,column=1)
